...
const FavouritesPage = () => {
  const { favoriteRecipes, toggleFavorite, searchRecipe } = useFoodContext();
  ...
  const visibleFavoriteRecipes = normalizedSearchQuery
    ? searchRecipe(normalizedSearchQuery).filter((recipe) => recipe.isFavorite)
    : favoriteRecipes;
        ...
        {visibleFavoriteRecipes.length === 0 ? (
          <div className="mx-auto mt-16 w-[min(92%,42rem)] rounded-2xl border border-dashed border-gray-700 bg-gray-800/60 px-6 py-12 text-center">
            ...
            <p className="mt-3 text-gray-400">
              {normalizedSearchQuery
                ? "Try a different search term to find saved recipes."
